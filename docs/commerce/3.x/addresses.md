...
Address lines are used, for example, on the [order edit](orders.md#editing-orders) page in the control panel. There are examples for [displaying an address](https://github.com/craftcms/commerce/blob/develop/example-templates/src/shop/_private/address/address.twig) in the [example templates](example-templates.md).
